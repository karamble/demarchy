...
What fires them: a new entry in the filtered set. The identity of a `br.messages` entry is `type`, `fromNick`, `text` and `gcid` together, so a repeated identical message is the same entry and does not ring again, while the same nick saying something else does. `~=` is a case-insensitive substring match, so `text~=invoice` catches "Invoice 42 attached". The second command replaces the identity with `--key fromNick`: the set is then the set of nicks, and it rings when a nick that was not in the previous sample writes. The list holds only the most recent messages, so a nick whose messages have all scrolled off counts as new again. A picture or file in a message reaches `text` folded to an icon and its caption or filename, such as `🖼️ receipt` or `📄 invoice-42.pdf`, and a Lightning invoice becomes `⚡ invoice 0.5 DCR`, so `text~=invoice` catches both a shared file by that name and a request to be paid.
